...
async function predictWithModel(text: string): Promise<MoodPrediction | null> {
    ...
    try {
        ...
        if (import.meta.env.MODE !== 'production') {
            console.debug('[MoodClassifier] model scores', allScores, 'text=', text);
        }
        ...
        return overridePredictionWithStrongKeywords(text, prediction);
    } catch (err) {
...
// ── Explicit keyword override for strong moods ─────────────────────────────────

const STRONG_OVERRIDES: Record<MoodType, string[]> = {
    happy: ['happy', 'joy', 'joyful', 'excited', 'amazing', 'great', 'fantastic', 'love', 'proud'],
    sad: ['sad', 'unhappy', 'depressed', 'tears', 'lonely', 'mourn', 'sorrow'],
    productive: ['productive', 'accomplished', 'achievement', 'success', 'finished', 'completed', 'organized', 'focused', 'motivated'],
    tired: ['tired', 'exhausted', 'fatigue', 'sleepy', 'burnout', 'sleep', 'drained', 'lethargic'],
    neutral: ['okay', 'ok', 'fine', 'alright', 'normal', 'meh', 'whatever'],
    angry: ['angry', 'mad', 'furious', 'rage', 'frustrated', 'annoyed', 'irritated', 'hate', 'hated', 'betrayal'],
};
...
function overridePredictionWithStrongKeywords(text: string, prediction: MoodPrediction): MoodPrediction {
    const lower = text.toLowerCase();

    for (const mood of MOOD_LABELS) {
        for (const keyword of STRONG_OVERRIDES[mood]) {
            const regex = new RegExp(`\\b${escapeRegExp(keyword)}\\b`, 'i');
            if (regex.test(lower)) {
                if (prediction.mood !== mood) {
                    if (import.meta.env.MODE !== 'production') {
                        console.debug('[MoodClassifier] overridden mood', prediction.mood, '->', mood, 'keyword=', keyword);
                    }
                    return {
                        mood,
                        confidence: Math.max(prediction.confidence, 0.65),
